    ...
    protected static function getParameters(\ReflectionMethod $method, array $parameters = null)
    {
        ...
        try {
            foreach ($method->getParameters() as $parameter) {
                switch (true) {
                    case Parameters::canBeInjectedParameter($parameter, $parameters):
                        $arguments[$parameter->getName()] = self::instantiation($parameters[$parameter->getName()]);
                        break;
                    case Parameters::sameParameter($parameter, $parameters):
                        $arguments[$parameter->getName()] = $parameters[$parameter->getName()];
                        break;
                    case null != $parameter->getClass() && self::container()->isInjected($parameter->getClass()->name, $method->getDocComment()):
                        ...
                        break;
                    case self::container()->isInstantiate($parameter->getClass()):
                        $arguments[$parameter->getName()] = self::instantiation($parameter->getClass()->name);
                        break;
                    case true != $parameter->isOptional():
